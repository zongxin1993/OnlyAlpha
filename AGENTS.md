...
7. 回测与实盘尽量共用同一策略接口、领域模型和事件模型；
8. 交易、行情、风控、缓存、存储、投研和 Web API 相互解耦；
9. 第一阶段支持中国 A 股；
10. 架构从第一天起预留期货、港股、美股、外汇、期权和数字资产能力；
11. 核心交易数值必须具备明确精度和约束；
12. 关键行为必须可测试、可观测、可恢复。
...
- 不修改 `/home/zongxin/workspace/MyQuant`；
- 不删除或覆盖 MyQuant 文件；
- 不启动真实交易；
- 不使用真实账户做验证；
