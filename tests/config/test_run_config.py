...
def test_yaml_and_json_normalize_to_the_same_run_config() -> None:
    yaml_config = OnlyRunConfig.load("examples/backtest_macd/config.yaml")
    json_config = OnlyRunConfig.load("examples/backtest_macd/config.json")
    ...
    assert yaml_config.strategies == json_config.strategies


def test_common_parser_accepts_every_runtime_type_without_reading_extensions() -> None:
    baseline = OnlyRunConfig.load("examples/backtest_macd/config.json")
