from onlyalpha.execution import (
    OnlyExecutionProjectionApplier,
    OnlyExecutionProjectionBatchStatus,
    OnlyInMemoryAppliedProjectionLedger,
)
from tests.execution.support.manager_authority_digest import only_test_runtime_authority_digest
from tests.execution.targets.support import only_test_projection_target_bundle


def test_all_twelve_targets_rebuild_a_lost_applied_projection_ledger() -> None:
    bundle = only_test_projection_target_bundle()
    assert bundle.apply_all().status is OnlyExecutionProjectionBatchStatus.COMPLETED
    before = only_test_runtime_authority_digest(bundle.environment)
    ledger = OnlyInMemoryAppliedProjectionLedger()
    recovered = OnlyExecutionProjectionApplier(bundle.create_targets(ledger)).apply(bundle.transaction)
    assert recovered.status is OnlyExecutionProjectionBatchStatus.COMPLETED
    assert len(recovered.recovered) == 12
    assert not recovered.applied
    assert not recovered.idempotent
    assert len(ledger.records()) == 12
    assert only_test_runtime_authority_digest(bundle.environment) == before
