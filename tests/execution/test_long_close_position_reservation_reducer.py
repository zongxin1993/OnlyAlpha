...
def test_long_close_projection_batch_contains_no_cash_reservations() -> None:
    ...
    assert OnlyExecutionProjectionComponent.POSITION_RESERVATION in components
    assert OnlyExecutionProjectionComponent.ACCOUNT_CASH_RESERVATION not in components
    assert OnlyExecutionProjectionComponent.STRATEGY_CASH_RESERVATION not in components
