...
def test_order_cumulative_minimum_is_charged_once_then_only_the_difference() -> None:
    ...
    assert tuple(item.application.total_charges.amount for item in (first, second, third)) == (
        Decimal("5.00"),
        Decimal("0"),
        Decimal("3.00"),
    )
    assert third.after.cumulative_charges.amount == Decimal("8.00")
...
def test_fill_scope_charges_each_fill_and_negative_cumulative_increment_fails_closed() -> None:
    ...
    assert tuple(item.application.total_charges.amount for item in fill) == (Decimal("5.00"), Decimal("5.00"))
    assert fill[-1].after.cumulative_charges.amount == Decimal("10.00")
