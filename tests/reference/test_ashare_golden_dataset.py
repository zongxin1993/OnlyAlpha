...
def test_frozen_reference_dataset_manifest_and_coverage() -> None:
    ...
    records = tuple(OnlyCnAshareInstrumentReference.from_mapping(item) for item in raw_records)
    authority = OnlyCnAshareReferenceAuthority.create(records)
    ...
    assert all(item.previous_close > 0 for item in records)
    assert authority.identity == OnlyCnAshareReferenceAuthority.create(tuple(reversed(records))).identity
