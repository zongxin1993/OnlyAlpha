...
class OnlyTushareConfig:
    ...
    def parse(cls, extensions: object) -> OnlyTushareConfig:
        if not isinstance(extensions, dict):
            raise OnlyTushareError(
                "TUSHARE_CONFIG_INVALID", "extensions must be a mapping"
            )
        allowed = {
            "token_env",
            "token",
        ...
        if unknown:
            raise OnlyTushareError(
                "TUSHARE_CONFIG_INVALID", f"unknown fields: {sorted(unknown)}"
            )
        if extensions.get("mode", "historical") != "historical":
            raise OnlyTushareError(
                "TUSHARE_CONFIG_INVALID", "only historical mode is supported"
            )
        adjustment = {
        ...
        if adjustment is None:
            raise OnlyTushareError(
                "TUSHARE_CONFIG_INVALID", "adjustment must be none, qfq, or hfq"
            )
        frequency = str(extensions.get("frequency", "1d")).lower()
        if frequency != "1d":
            raise OnlyTushareError(
                "TUSHARE_UNSUPPORTED_BAR_TYPE", "only daily Bars are supported"
            )
    ...
    def resolve_token(self) -> str:
        ...
        if not token:
            raise OnlyTushareError(
                "TUSHARE_TOKEN_MISSING", "Tushare credential is not configured"
            )
        return token
