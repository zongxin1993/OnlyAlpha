...
def test_real_tushare_daily_cache_roundtrip(tmp_path, instrument, calendar, bar_type) -> None:
    ...
    provider = OnlyTushareHistoricalDataProvider("tushare-real", instrument, calendar, create_client)
    requested = OnlyTimeRange(datetime(2025, 1, 1, tzinfo=UTC), datetime(2025, 1, 11, tzinfo=UTC))
    ...
    assert tuple(item.ts_event for item in first.records) == tuple(sorted(item.ts_event for item in first.records))
