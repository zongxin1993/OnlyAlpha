...
def test_real_history_converts_to_onlyalpha_bars() -> None:
    instrument = OnlyInstrumentId.parse(
        os.environ.get("ONLYALPHA_MINIQMT_SYMBOL", "600000.XSHG")
    )
    ...
    start = datetime.fromisoformat(
        os.environ.get("ONLYALPHA_MINIQMT_START", "2026-07-17T01:30:00+00:00")
    )
    end = datetime.fromisoformat(
        os.environ.get("ONLYALPHA_MINIQMT_END", "2026-07-17T02:01:00+00:00")
    )
    ...
    assert records
    assert tuple(item.ts_event for item in records) == tuple(
        sorted(item.ts_event for item in records)
    )
