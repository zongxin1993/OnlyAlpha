...
class QualityPolicy:
    ...
    certification_required_gates: frozenset[str]
...
def load_quality_policy(path: Path = QUALITY_POLICY_PATH) -> QualityPolicy:
    ...
    if configured_schema_version != 1:
    ...
    certification_required = _require_gate_set(certification, "required_gates")
    if quality_required & event_lanes:
        raise ValueError("quality required gates and event lane gates must be disjoint")
    if "coverage" in quality_required or "coverage" in certification_required:
    ...
    if owner not in quality_required or owner not in certification_required:
        raise ValueError("historical evidence owner must be mandatory in quality and certification")
    ...
    return QualityPolicy(
        ...
        certification_required_gates=certification_required,
