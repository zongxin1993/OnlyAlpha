...
3. `OnlyExecutionPrecondition.expected_state_hash` 可选，Projection Identity 没有 expected/result state hash，参考 Apply 只检查 version 与 applied payload；State Hash 未参与安装。
...
5. SQLite commit 捕获任意 `sqlite3.IntegrityError` 后统一查询幂等事务；无匹配事务时仍抛 `OnlyExecutionTransactionConflict`，导致 trigger abort、outbox failure 和普通 integrity failure 被误分类。
