...
Backtest Runtime 通过 `process_trade(fill_update, position_trade)` 校验同一 Runtime/Order/Cluster/Account/Instrument/Fill，
然后同步执行 Order Update、Position、Allocation、Strategy Ledger、Valuation、Reservation 终态和 Event drain。
该入口不撮合、不连接 Gateway，也不从 Placeholder 生成成交。`settle_positions()` 只接受 Calendar 推导的 TradingDay。
