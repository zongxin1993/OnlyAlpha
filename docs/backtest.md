...
其中受支持的 Generic T0 Cash LIMIT BUY OPEN 每个 whole/partial Fill 细化为 `Pure Planner → Prepared Transaction → durable Transaction Store commit → ordered incremental Projection Targets → Projection Ready → at-least-once Outbox`。SELL/CLOSE、Virtual Broker Partial Fill Schedule、完整 Multi-Fill Recovery、Futures/Margin 和多 Cluster 固定资金归约仍是明确的后续迁移边界。
