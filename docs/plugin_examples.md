...
ADR 0110, ADR 0129 and ADR 0131 define the active boundary. Public reusable Operators and Indicators live in official plugins.
Production Alpha and Strategy assets are private database-native authoring assets; Git repositories and distributions are optional
...
Seeds are imported through the Private Asset Authority before validation or execution. Alpha `source.py` becomes Revision content only;
Strategy `strategy.json` becomes a structured Draft/Revision. Neither repository path is execution or Runtime Authority.
...
they are not the required Private Alpha/Strategy production authoring workflow. ADR 0129 defines PostgreSQL-backed Draft/Revision
authoring, exact Alpha API binding, and structured Strategy definitions;
no Core component recursively executes arbitrary paths or treats a package location as asset/runtime identity.

Public Operator/Indicator distributions expose `onlyalpha.quant_assets` providers. A validated Private Alpha Revision derives a
