"""Exact validation, artifacts and execution bindings for DB-native Private Alpha V1."""
...
from onlyalpha.quant_assets.private import OnlyPrivateAlphaRevision, only_private_alpha_source_sha256
...
_ADAPTER_FINGERPRINTS = {
    "RESEARCH": only_canonical_fingerprint({"adapter": "ONLYALPHA_PRIVATE_ALPHA_RESEARCH_V1"}),
    "TRADING": only_canonical_fingerprint({"adapter": "ONLYALPHA_PRIVATE_ALPHA_TRADING_V1"}),
}

ONLY_PRIVATE_ALPHA_NUMERIC_V1 = OnlyNumericDefinition(
...
class OnlyPrivateAlphaApiContractV1:
    api_version: int = 1
    contract_name: str = "ONLYALPHA_PRIVATE_ALPHA_API_V1"
        ...
        return only_canonical_fingerprint(
            {
                ...
                "numeric": {
                    "representation": ONLY_PRIVATE_ALPHA_NUMERIC_V1.representation,
                    "precision": ONLY_PRIVATE_ALPHA_NUMERIC_V1.precision,
                    "output_quantum": str(ONLY_PRIVATE_ALPHA_NUMERIC_V1.output_quantum),
                    "rounding": ONLY_PRIVATE_ALPHA_NUMERIC_V1.rounding,
                },
...
ONLY_PRIVATE_ALPHA_API_V1 = OnlyPrivateAlphaApiContractV1()


@dataclass(frozen=True, slots=True)
class OnlyPrivateAlphaValidationPolicyV1:
    policy_name: str = "ONLYALPHA_PRIVATE_ALPHA_VALIDATION_V1"
...
ONLY_PRIVATE_ALPHA_VALIDATION_POLICY_V1 = OnlyPrivateAlphaValidationPolicyV1()


class OnlyPrivateAlphaValidationDisposition(StrEnum):
    PASS = "PASS"
    FAIL = "FAIL"


@dataclass(frozen=True, slots=True)
class OnlyPrivateAlphaValidationEvidenceV1:
    ...
    validation_disposition: OnlyPrivateAlphaValidationDisposition
    errors: tuple[str, ...] = ()
    ...
    def __post_init__(self) -> None:
        for value in (
            ...
            self.source_sha256,
            self.alpha_api_contract_fingerprint,
            ...
            if not _is_sha(value):
                raise ValueError("PRIVATE_ALPHA_VALIDATION_EVIDENCE_INVALID")
        if (self.validation_disposition is OnlyPrivateAlphaValidationDisposition.PASS) == bool(self.errors):
            raise ValueError("PRIVATE_ALPHA_VALIDATION_EVIDENCE_INVALID")
...
class _SourcePolicy(ast.NodeVisitor):
    ...
    def generic_visit(self, node: ast.AST) -> None:
        if not isinstance(node, self._allowed):
            raise ValueError(f"PRIVATE_ALPHA_FORBIDDEN_AST:{type(node).__name__}")
    ...
    def visit_Name(self, node: ast.Name) -> None:
        if node.id.startswith("__") or node.id in {
            ...
            raise ValueError("PRIVATE_ALPHA_FORBIDDEN_NAME")


def only_validate_private_alpha_revision(revision: OnlyPrivateAlphaRevision) -> OnlyPrivateAlphaValidationEvidenceV1:
    errors: list[str] = []
    if revision.alpha_api_version != 1:
        errors.append("PRIVATE_ALPHA_API_VERSION_UNSUPPORTED")
    if revision.alpha_api_contract_fingerprint != ONLY_PRIVATE_ALPHA_API_V1.api_contract_fingerprint:
        errors.append("PRIVATE_ALPHA_API_CONTRACT_MISMATCH")
    if revision.source_sha256 != only_private_alpha_source_sha256(revision.source_text):
        errors.append("PRIVATE_ALPHA_SOURCE_HASH_MISMATCH")
    try:
        ...
        if (
            ...
            raise ValueError("PRIVATE_ALPHA_ENTRYPOINT_INVALID")
        ...
        if (
            ...
            raise ValueError("PRIVATE_ALPHA_ENTRYPOINT_SIGNATURE_INVALID")
        _SourcePolicy().visit(tree)
    except (SyntaxError, ValueError) as exc:
        errors.append(str(exc))
    return OnlyPrivateAlphaValidationEvidenceV1(
        revision.revision_fingerprint,
        revision.source_sha256,
        revision.alpha_api_version,
        revision.alpha_api_contract_fingerprint,
        ONLY_PRIVATE_ALPHA_VALIDATION_POLICY_V1.validation_policy_fingerprint,
        ...
        OnlyPrivateAlphaValidationDisposition.FAIL if errors else OnlyPrivateAlphaValidationDisposition.PASS,
        tuple(errors),
    )


@dataclass(frozen=True, slots=True)
class OnlyPrivateAlphaSourceArtifactManifestV1:
    alpha_id: str
    ...
    @classmethod
    def from_dict(cls, payload: Mapping[str, object]) -> OnlyPrivateAlphaSourceArtifactManifestV1:
    ...
    @classmethod
    def materialize(
        cls, revision: OnlyPrivateAlphaRevision, evidence: OnlyPrivateAlphaValidationEvidenceV1
    ) -> tuple[OnlyPrivateAlphaSourceArtifactManifestV1, bytes]:
        if (
            evidence != only_validate_private_alpha_revision(revision)
            or evidence.validation_disposition is not OnlyPrivateAlphaValidationDisposition.PASS
            or evidence.revision_fingerprint != revision.revision_fingerprint
            or evidence.source_sha256 != revision.source_sha256
            or evidence.alpha_api_contract_fingerprint != revision.alpha_api_contract_fingerprint
            or evidence.validation_policy_fingerprint
            != ONLY_PRIVATE_ALPHA_VALIDATION_POLICY_V1.validation_policy_fingerprint
        ):
            raise ValueError("PRIVATE_ALPHA_VALIDATION_EVIDENCE_MISMATCH")
    ...
    def verify(self, source: bytes) -> None:
        if (
            ...
            or self.alpha_api_version != 1
            or self.alpha_api_contract_fingerprint != ONLY_PRIVATE_ALPHA_API_V1.api_contract_fingerprint
        ):
            raise ValueError("PRIVATE_ALPHA_SOURCE_ARTIFACT_MISMATCH")
...
def _execute_child(
    ...
    try:
        ...
        exec(compile(source, "<private-alpha-artifact>", "exec"), namespace, namespace)
        calculate = cast(Callable[[object, object, object], object], namespace["calculate"])
        result = calculate(OnlyCanonicalValueSemanticsV1(ONLY_PRIVATE_ALPHA_NUMERIC_V1), dict(inputs), dict(parameters))
...
@dataclass(frozen=True, slots=True)
class OnlyPrivateAlphaIsolatedProgramHost:
    timeout_seconds: float = 1.0

    def execute(
        self,
        manifest: OnlyPrivateAlphaSourceArtifactManifestV1,
        source: bytes,
        ...
        if not parent.poll(self.timeout_seconds):
            process.kill()
            process.join()
            raise TimeoutError("PRIVATE_ALPHA_EXECUTION_TIMEOUT")
        success, result = parent.recv()
        process.join()
        if not success or process.exitcode != 0:
            raise ValueError(f"PRIVATE_ALPHA_EXECUTION_FAILED:{result}")
        return result


@dataclass(frozen=True, slots=True)
class OnlyPrivateAlphaAdapterV1:
    backend: str
    ...
    def __post_init__(self) -> None:
        if self.backend not in {"RESEARCH", "TRADING"} or not _is_sha(self.adapter_fingerprint):
            raise ValueError("PRIVATE_ALPHA_ADAPTER_INVALID")

    def implementation_fingerprint(self, manifest: OnlyPrivateAlphaSourceArtifactManifestV1) -> str:
        return only_canonical_fingerprint(
            {
                ...
                "alpha_api_contract_fingerprint": manifest.alpha_api_contract_fingerprint,
...
@dataclass(frozen=True, slots=True)
class OnlyPrivateAlphaDefinitionResolverV1:
...
@dataclass(frozen=True, slots=True)
class OnlyPrivateAlphaResearchBackendV1:
    artifact: OnlyPrivateAlphaSourceArtifactManifestV1
    source: bytes
    adapter: OnlyPrivateAlphaAdapterV1
    ...
    def execute(
        ...
        if set(inputs) != {item.name for item in definition.inputs}:
            raise ValueError("PRIVATE_ALPHA_RESEARCH_INPUT_MISMATCH")
...
@dataclass(frozen=True, slots=True)
class OnlyPrivateAlphaTradingBackendV1:
    definition: OnlyCalculationDefinition
    artifact: OnlyPrivateAlphaSourceArtifactManifestV1
    source: bytes
    adapter: OnlyPrivateAlphaAdapterV1
    ...
    def update(self, inputs: Mapping[str, object]) -> Mapping[str, object]:
        ...
        if not isinstance(result, Mapping):
            raise ValueError("PRIVATE_ALPHA_OUTPUT_MISMATCH")
        return _normalize_output(cast(Mapping[str, object], result), self.definition.outputs)


@dataclass(frozen=True, slots=True)
class OnlyPrivateAlphaTradingBackendFactoryV1:
    artifact: OnlyPrivateAlphaSourceArtifactManifestV1
    source: bytes
    adapter: OnlyPrivateAlphaAdapterV1

    def create(self, definition: OnlyCalculationDefinition, request: object) -> OnlyPrivateAlphaTradingBackendV1:
        del request
        return OnlyPrivateAlphaTradingBackendV1(definition, self.artifact, self.source, self.adapter)


def only_private_alpha_type_definition(revision: OnlyPrivateAlphaRevision) -> OnlyCalculationTypeDefinition:
    return OnlyCalculationTypeDefinition(
        ...
        ONLY_PRIVATE_ALPHA_NUMERIC_V1,
        OnlyFactorKind.TIME_SERIES,
    )


def only_private_alpha_backend_registrations(
    revision: OnlyPrivateAlphaRevision,
    artifact: OnlyPrivateAlphaSourceArtifactManifestV1,
    source: bytes,
    research: OnlyPrivateAlphaAdapterV1,
    trading: OnlyPrivateAlphaAdapterV1,
    ...
    if artifact.revision_fingerprint != revision.revision_fingerprint:
        raise ValueError("PRIVATE_ALPHA_SOURCE_ARTIFACT_MISMATCH")
    definition = only_private_alpha_type_definition(revision)
    resolver = OnlyPrivateAlphaDefinitionResolverV1(definition)
    ...
    dependencies = (
        OnlyCalculationSemanticDependency("onlyalpha.private-alpha.api", "1", artifact.alpha_api_contract_fingerprint),
        OnlyCalculationSemanticDependency(
            "onlyalpha.private-alpha.source-artifact", "1", artifact.source_artifact_fingerprint
        ),
    )

    def implementation(
        backend: OnlyCalculationBackendKind, adapter: OnlyPrivateAlphaAdapterV1
    ) -> OnlyCalculationImplementationManifest:
        return only_implementation_manifest_from_bytes(
            calculation_type_reference=reference,
            backend_kind=backend,
            entrypoint_identity=(
                "onlyalpha.quant_assets.private_alpha_execution:OnlyPrivateAlphaResearchBackendV1"
                if backend is OnlyCalculationBackendKind.RESEARCH
                else "onlyalpha.quant_assets.private_alpha_execution:OnlyPrivateAlphaTradingBackendFactoryV1"
            ),
            resources={
                "private_alpha/source.py": source,
                f"private_alpha/{backend.value.lower()}-adapter.py": adapter_source,
                f"private_alpha/{backend.value.lower()}-identity.txt": adapter.adapter_fingerprint.encode(),
            },
    ...
    return (
        OnlyCalculationBackendRegistration(
            ...
            OnlyPrivateAlphaResearchBackendV1(artifact, source, research),
            resolver,
        ...
        OnlyCalculationBackendRegistration(
            ...
            OnlyPrivateAlphaTradingBackendFactoryV1(artifact, source, trading),
            resolver,
...
def _contract(value: object, name: str) -> Mapping[str, object]:
    if not isinstance(value, Mapping) or any(not isinstance(key, str) for key in value):
        raise ValueError(f"PRIVATE_ALPHA_{name.upper()}_CONTRACT_INVALID")
...
def _data_type(value: Mapping[str, object]) -> OnlyCalculationDataType:
    try:
        ...
        raise ValueError("PRIVATE_ALPHA_DATA_TYPE_INVALID") from exc
...
def _input(name: str, value: object) -> OnlyInputDefinition:
    ...
    if not {"type"} <= set(contract) <= {"type", "nullable", "missing"} or {
        ...
        raise ValueError("PRIVATE_ALPHA_INPUT_CONTRACT_INVALID")
...
def _output(name: str, value: object) -> OnlyOutputDefinition:
    ...
    if not {"type"} <= set(contract) <= {"type", "nullable", "missing", "semantic_type"} or {
        ...
        raise ValueError("PRIVATE_ALPHA_OUTPUT_CONTRACT_INVALID")
...
def _parameter(name: str, value: object) -> OnlyParameterDefinition:
    ...
    if not {"type"} <= set(contract) <= {"type", "required", "default"}:
        raise ValueError("PRIVATE_ALPHA_PARAMETER_CONTRACT_INVALID")
    try:
        ...
        raise ValueError("PRIVATE_ALPHA_PARAMETER_CONTRACT_INVALID") from exc
...
def _nullable(contract: Mapping[str, object], owner: str) -> bool:
    ...
    if not isinstance(value, bool):
        raise ValueError(f"PRIVATE_ALPHA_{owner}_CONTRACT_INVALID")
    return value
...
def _normalize_output(values: Mapping[str, object], outputs: tuple[OnlyOutputDefinition, ...]) -> Mapping[str, object]:
    ...
    try:
        ...
        raise ValueError("PRIVATE_ALPHA_OUTPUT_MISMATCH") from exc
...
@dataclass(frozen=True, slots=True)
class OnlyPrivateAlphaResearchTradingEquivalenceEvidenceV1:
    revision_fingerprint: str
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ValueError("PRIVATE_ALPHA_EQUIVALENCE_EVIDENCE_MISMATCH")
    ...
    @classmethod
    def from_dict(cls, payload: Mapping[str, object]) -> OnlyPrivateAlphaResearchTradingEquivalenceEvidenceV1:
        fields = set(cls.__dataclass_fields__)
    ...
    def certify(
        ...
        if set(by_backend) != {OnlyCalculationBackendKind.RESEARCH, OnlyCalculationBackendKind.TRADING} or not vectors:
            raise ValueError("PRIVATE_ALPHA_EQUIVALENCE_ADAPTER_MISMATCH")
        ...
        if (
            ...
            or not isinstance(research_registration.provider, OnlyPrivateAlphaResearchBackendV1)
            or not isinstance(trading_registration.provider, OnlyPrivateAlphaTradingBackendFactoryV1)
        ):
            raise ValueError("PRIVATE_ALPHA_EQUIVALENCE_ADAPTER_MISMATCH")
        ...
        if any(set(vector) != set(expected_inputs) for vector in vectors):
            raise ValueError("PRIVATE_ALPHA_EQUIVALENCE_INPUT_MISMATCH")
        research_columns = {
        ...
        if research_fp != trading_fp:
            raise ValueError("PRIVATE_ALPHA_EQUIVALENCE_FAILED")
        ...
        return cls(
            ...
            manifest.alpha_api_contract_fingerprint,
...
class OnlyPrivateAlphaProviderSnapshotEntryV1:
    alpha_id: str
    semantic_version: str
    revision_fingerprint: str
    source_sha256: str
    ...
    def derive(
        cls,
        manifest: OnlyPrivateAlphaSourceArtifactManifestV1,
        evidence: OnlyPrivateAlphaResearchTradingEquivalenceEvidenceV1,
        ...
        if (
            ...
            or evidence.alpha_api_contract_fingerprint != manifest.alpha_api_contract_fingerprint
    ...
    @classmethod
    def from_dict(cls, payload: Mapping[str, object]) -> OnlyPrivateAlphaProviderSnapshotEntryV1:
        if set(payload) != set(cls.__dataclass_fields__):
            raise ValueError("PRIVATE_ALPHA_PROVIDER_SNAPSHOT_INVALID")
...
@dataclass(frozen=True, slots=True)
class OnlyPrivateAlphaProviderSnapshotV1:
    entries: tuple[OnlyPrivateAlphaProviderSnapshotEntryV1, ...]

    def __post_init__(self) -> None:
        canonical = tuple(sorted(self.entries))
        if not canonical or len({item.alpha_id for item in canonical}) != len(canonical):
            raise ValueError("PRIVATE_ALPHA_PROVIDER_SNAPSHOT_INVALID")
    ...
    def snapshot_fingerprint(self) -> str:
        return only_canonical_fingerprint(
            {
                "contract": "ONLYALPHA_PRIVATE_ALPHA_PROVIDER_SNAPSHOT_V1",
    ...
    @classmethod
    def from_dict(cls, payload: Mapping[str, object]) -> OnlyPrivateAlphaProviderSnapshotV1:
        if set(payload) != {"entries", "snapshot_fingerprint"} or not isinstance(payload["entries"], list):
            raise ValueError("PRIVATE_ALPHA_PROVIDER_SNAPSHOT_INVALID")
        result = cls(
            tuple(
                OnlyPrivateAlphaProviderSnapshotEntryV1.from_dict(cast(Mapping[str, object], item))
...
class OnlyPrivateAlphaExecutableClosureV1:
    revision: OnlyPrivateAlphaRevision
    validation_evidence: OnlyPrivateAlphaValidationEvidenceV1
    source_artifact: OnlyPrivateAlphaSourceArtifactManifestV1
    source: bytes
    registrations: tuple[OnlyCalculationBackendRegistration, OnlyCalculationBackendRegistration]
    equivalence_evidence: OnlyPrivateAlphaResearchTradingEquivalenceEvidenceV1
    provider_snapshot: OnlyPrivateAlphaProviderSnapshotV1
    certification_vectors: tuple[Mapping[str, object], ...]
    certification_parameters: Mapping[str, object]

    def __init__(self) -> None:
        raise TypeError("PRIVATE_ALPHA_EXECUTABLE_CLOSURE_CANONICAL_PRODUCER_REQUIRED")

    def verify_canonical(self) -> None:
        validation = only_validate_private_alpha_revision(self.revision)
        artifact, source = OnlyPrivateAlphaSourceArtifactManifestV1.materialize(self.revision, validation)
        ...
        if (
            type(self) is not OnlyPrivateAlphaExecutableClosureV1
            or validation != self.validation_evidence
            or artifact != self.source_artifact
            or source != self.source
            or research is None
            or trading is None
            or type(research.provider) is not OnlyPrivateAlphaResearchBackendV1
            or type(trading.provider) is not OnlyPrivateAlphaTradingBackendFactoryV1
            or type(research.provider.adapter) is not OnlyPrivateAlphaAdapterV1
            or type(trading.provider.adapter) is not OnlyPrivateAlphaAdapterV1
            or type(research.provider.adapter.host) is not OnlyPrivateAlphaIsolatedProgramHost
            or type(trading.provider.adapter.host) is not OnlyPrivateAlphaIsolatedProgramHost
            ...
            raise ValueError("PRIVATE_ALPHA_EXECUTABLE_CLOSURE_MISMATCH")
        ...
        equivalence = OnlyPrivateAlphaResearchTradingEquivalenceEvidenceV1.certify(
            artifact,
            source,
            registrations,
        ...
        snapshot = OnlyPrivateAlphaProviderSnapshotV1(
            (OnlyPrivateAlphaProviderSnapshotEntryV1.derive(artifact, equivalence),)
        )
        if (
    ...
    def create(
        ...
        parameters: Mapping[str, object],
        *,
        host: OnlyPrivateAlphaIsolatedProgramHost | None = None,
    ) -> OnlyPrivateAlphaExecutableClosureV1:
        validation = only_validate_private_alpha_revision(revision)
        artifact, source = OnlyPrivateAlphaSourceArtifactManifestV1.materialize(revision, validation)
        isolated = host or OnlyPrivateAlphaIsolatedProgramHost()
        research = OnlyPrivateAlphaAdapterV1(
            "RESEARCH",
        ...
        registrations = only_private_alpha_backend_registrations(revision, artifact, source, research, trading)
        ...
        if research_manifest is None or trading_manifest is None:
            raise ValueError("PRIVATE_ALPHA_IMPLEMENTATION_IDENTITY_MISSING")
        equivalence = OnlyPrivateAlphaResearchTradingEquivalenceEvidenceV1.certify(
            artifact,
            source,
            registrations,
