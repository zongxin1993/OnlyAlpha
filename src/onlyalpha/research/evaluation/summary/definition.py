"""Versioned Summary Statistics mathematical definitions."""
...
@dataclass(frozen=True, slots=True)
class OnlyResearchCoverageSummaryDefinition:
    source_method: OnlyResearchStatisticsMethod
    summary_kind: OnlyResearchSummaryKind = OnlyResearchSummaryKind.COVERAGE_SUMMARY
    coverage_semantics: OnlyResearchCoverageSemantics = OnlyResearchCoverageSemantics.OBSERVED_TIMESTAMP_PAIR
    numeric: OnlyNumericDefinition = OnlyNumericDefinition("DECIMAL", 38, Decimal("0.000000000001"), "ROUND_HALF_EVEN")
    decimal_execution_policy: str = "onlyalpha.decimal.execution@1"
    schema_version: int = RESEARCH_COVERAGE_SUMMARY_DEFINITION_SCHEMA_VERSION

    def __post_init__(self) -> None:
        if self.schema_version != RESEARCH_COVERAGE_SUMMARY_DEFINITION_SCHEMA_VERSION:
            raise ValueError("unsupported Coverage Summary Definition schema version")
        if not isinstance(self.source_method, OnlyResearchStatisticsMethod) or self.source_method not in {
            OnlyResearchStatisticsMethod.IC,
            OnlyResearchStatisticsMethod.RANK_IC,
        }:
            raise ValueError("Coverage Summary source method is unsupported")
        if self.summary_kind is not OnlyResearchSummaryKind.COVERAGE_SUMMARY:
            raise ValueError("Coverage Summary kind is invalid")
        if self.coverage_semantics is not OnlyResearchCoverageSemantics.OBSERVED_TIMESTAMP_PAIR:
            raise ValueError("Coverage Summary semantics are unsupported")
        if self.numeric != OnlyNumericDefinition("DECIMAL", 38, Decimal("0.000000000001"), "ROUND_HALF_EVEN"):
            raise ValueError("Coverage Summary requires Decimal(38), quantum 1e-12, ROUND_HALF_EVEN")
        policy = ONLY_DECIMAL_EXECUTION_POLICY_V1
        if self.decimal_execution_policy != f"{policy.policy_id}@{policy.semantic_version}":
            raise ValueError("Coverage Summary Decimal execution policy is unsupported")
    ...
    @classmethod
    def from_dict(cls, payload: Mapping[str, object]) -> OnlyResearchCoverageSummaryDefinition:
        expected = {
            "schema_version",
            "summary_kind",
            "source_method",
        ...
        if set(payload) != expected:
            raise ValueError("Coverage Summary Definition fields are invalid")
        numeric = payload["numeric"]
        if not isinstance(numeric, Mapping) or set(numeric) != {
            "representation",
            "precision",
            ...
            raise ValueError("Coverage Summary numeric fields are invalid")
        return cls(
            source_method=OnlyResearchStatisticsMethod(_string(payload, "source_method")),
            summary_kind=OnlyResearchSummaryKind(_string(payload, "summary_kind")),
            coverage_semantics=OnlyResearchCoverageSemantics(_string(payload, "coverage_semantics")),
            numeric=OnlyNumericDefinition(
                _string(numeric, "representation"),
                _integer(numeric, "precision"),
                Decimal(_string(numeric, "output_quantum")),
